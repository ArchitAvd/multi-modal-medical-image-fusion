...
load(model);
[conv1_patchsize2,conv1_filters] = size(weights_b1_1);  
conv1_patchsize = sqrt(conv1_patchsize2);
[conv2_channels,conv2_patchsize2,conv2_filters] = size(weights_b1_2);  
conv2_patchsize = sqrt(conv2_patchsize2);
[conv3_channels,conv3_patchsize2,conv3_filters] = size(weights_b1_3);  
conv3_patchsize = sqrt(conv3_patchsize2);
[conv4_channels,conv4_patchsize2,conv4_filters] = size(weights_output); 
...
ir_dir ='./MyDatasets/CT-MRI/test/CT'; % CT Scans
vi_dir = './MyDatasets/CT-MRI/test/MRI'; % MRI Scans
for numii =17:17
disp(numii);
%% load images
